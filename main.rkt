...
(begin-for-syntax
  ...
  (define (mark stx)
    (syntax-case stx (filter map range)
      [(filter pred ls)
       (let-values ([(node def src) (mark #'ls)])
         (define/with-syntax (failer p) (generate-temporaries '(failer p)))
         (values (Filter #'p (node-out node) (node-dep* node) #'failer node)
                 (cons (cons #'p #'pred) def)
                 src))]
      [(map f ls ls* ...)
       (with-syntax ([(p o) (generate-temporaries '(p o))])
      ...
      [(range start stop step)
       (with-syntax ([(s me i e d) (generate-temporaries '(s me i e d))])
         (define here (Source #'s #'me #'(>= s e) #'s #'(+ 1 s)))
         (values here
                 (list (cons #'i #'start) (cons #'e #'stop) (cons #'d #'step))
                 (list (list #'s #'i here))))]
      [expr
       (with-syntax ([(l s me) (generate-temporaries '(l s me))])
  ...
  (define (trans stx)
    (syntax-case stx (map filter for-each foldl foldr andmap ormap)
      ...
      [(andmap f ls ls* ...)
       (let-values ([(n d src) (mark #'(map f ls ls* ...))])
         (match-define (Map fun in* _ dep* children) n)
         (define/with-syntax (i l) (generate-temporaries '(i l)))
      ...
      [(foldr f init ls ls* ...)
       (let-values ([(n d src) (mark #'(map f ls ls* ...))])
         (match-define (Map fun in* _ dep* children) n)
         (define/with-syntax (i) (generate-temporaries '(i)))
